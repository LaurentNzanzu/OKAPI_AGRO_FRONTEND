...
const FicheOrganisation = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.message || t('organisations.deleteError'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleSuspendre = async () => {
    ...
    try {
      await api.post(`/organisations/${id}/suspendre/`, { motif: motif.trim() });
  ...
  const handleReactiver = async () => {
    if (!window.confirm(t('organisations.reactiverConfirm'))) return;
    try {
      await api.post(`/organisations/${id}/reactiver/`);
      await fetchData();
    } catch (err) {
      alert(err.response?.data?.message || t('organisations.deleteError'));
    }
  ...
  if (error) {
    return (
      <AppPage>
        <div className="p-4 rounded-lg bg-danger/10 text-danger">{error}</div>
      </AppPage>
    );
  }
  ...
  return (
    <AppPage>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-page-subtitle text-gray-500 dark:text-slate-400">
              <span className="font-mono font-semibold">{organisation.code}</span> •{' '}
              <span className={statutColor}>
                {t(`organisations.statut${organisation.statut.charAt(0) + organisation.statut.slice(1).toLowerCase()}`)}
              </span>
            </p>
          </div>
        </div>
            ...
            {!editing ? (
              <>
                ...
                {organisation.statut === 'ACTIF' ? (
                  ...
                  <Button variant="success" onClick={handleReactiver}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card title={t('organisations.infoGenerales')} className="lg:col-span-2">
          {editing ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={formData.plan_abonnement || 'BASIC'}
                  onChange={(e) => setFormData({ ...formData, plan_abonnement: e.target.value })}
              ...
              <Input
                ...
                type="number"
                value={formData.quota_vehicules || 0}
                onChange={(e) => setFormData({ ...formData, quota_vehicules: parseInt(e.target.value) || 0 })}
              />
              <Input
                label={t('organisations.quotaChauffeurs')}
                type="number"
                value={formData.quota_chauffeurs || 0}
                onChange={(e) => setFormData({ ...formData, quota_chauffeurs: parseInt(e.target.value) || 0 })}
              />
              <Input
                label={t('organisations.quotaMissions')}
                type="number"
                value={formData.quota_missions_mois || 0}
                onChange={(e) => setFormData({ ...formData, quota_missions_mois: parseInt(e.target.value) || 0 })}
              />
              <Input
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-sm">
              <InfoRow label={t('organisations.colNom')} value={organisation.nom} />
              <InfoRow label={t('organisations.colEmail')} value={organisation.email_admin} />
              <InfoRow label={t('organisations.colPlan')} value={organisation.plan_abonnement} />
              <InfoRow label={t('organisations.devise')} value={organisation.devise} />
              <InfoRow label={t('organisations.dateDebut')} value={organisation.date_debut || '—'} />
              <InfoRow label={t('organisations.dateFin')} value={organisation.date_fin || '—'} />
            </dl>
          )}
        </Card>
        ...
        <Card title={t('organisations.quotas')}>
          <div className="space-y-4">
            <QuotaRow label={t('organisations.quotaVehicules')} value={organisation.quota_vehicules} />
            <QuotaRow label={t('organisations.quotaChauffeurs')} value={organisation.quota_chauffeurs} />
            <QuotaRow label={t('organisations.quotaMissions')} value={organisation.quota_missions_mois} />
          </div>
        </Card>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md bg-surface-light dark:bg-surface-dark rounded-xl shadow-dropdown border border-border-light dark:border-border-dark p-6">
            ...
            <textarea
              ...
              placeholder={t('organisations.suspendreMotifPlaceholder')}
              className="w-full px-4 py-2.5 rounded-lg border border-border-light dark:border-border-dark bg-white dark:bg-surface-dark text-sm focus:outline-none focus:ring-2 focus:ring-primary-600"
            />
            <div className="mt-5 flex justify-end gap-2">
              ...
              <Button variant="danger" disabled={!motif.trim()} onClick={handleSuspendre}>
                {t('organisations.suspendreConfirm')}
              </Button>
            </div>
          </div>
        </div>
...
const InfoRow = ({ label, value }) => (
  <div>
    ...
    <dd className="mt-1 text-sm font-medium text-gray-900 dark:text-slate-100">{value}</dd>
  </div>
);
